...
func tagSSIParser(doc *Parser, start *Token, arguments *Parser) (INodeTag, *Error) {
	...
	if fileToken := arguments.MatchType(TokenString); fileToken != nil {
		...
		if arguments.Match(TokenIdentifier, "parsed") != nil {
			// parsed
			temporaryTpl, err := doc.template.set.FromFile(doc.template.set.Resolver.Resolve(doc.template, fileToken.Val))
			if err != nil {
			...
			// plaintext
			buf, err := ioutil.ReadFile(doc.template.set.Resolver.Resolve(doc.template, fileToken.Val))
			if err != nil {
				return nil, (&Error{
					Sender:   "tag:ssi",
					ErrorMsg: err.Error(),
